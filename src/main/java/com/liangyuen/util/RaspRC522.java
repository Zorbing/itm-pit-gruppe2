 ...
 * 
 * source: https://github.com/LiangYuen/Pi4j-RC522
 */
public class RaspRC522
{
	...
	private static final int NRSTPD_DEFAULT = 22;
	private static final int SPEED_MIN = 500000;
	private static final int SPEED_MAX = 32000000;
	private static final int SPEED_DEFAULT = SPEED_MIN;
	
	private int NRSTPD = NRSTPD_DEFAULT;
	...
	public RaspRC522(int speed, int pinReset)
	{
		...
		{
			throw new IndexOutOfBoundsException("Speed out of range.");
		}
		else
		{
			this.speed = speed;
	...
	public RaspRC522()
	{
		this(SPEED_DEFAULT, NRSTPD_DEFAULT);
	}
...
//		Gpio.wiringPiSetup();
		...
		if (fd <= -1)
		{
			throw new Exception("Failed to set up SPI communication.");
		}
	...
	private void Write_RC522(byte address, byte value)
	{
		...
		if (result == -1)
		{
			System.err.println("Device write error, address=" + address + ", value=" + value);
		}
	}
	...
	private byte Read_RC522(byte address)
	{
		...
		if (result == -1)
		{
			System.err.println("Device read error, address=" + address);
		}
		return data[1];
	}
	...
	public int Request(byte req_mode, int[] back_bits) // 1
	{
		int status;
		...
		{
			// System.out.println("status=" + status + ", back_bits[0]=" +
			// back_bits[0]);
			status = MI_ERR;
		}
	...
	// back_data-5 4 tagid+1
	public int AntiColl(byte[] back_data)
	{
		int status;
		byte[] serial_number = new byte[2]; // 2
